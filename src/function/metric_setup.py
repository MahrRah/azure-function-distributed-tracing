...
from settings import MonitorQueueInformation
...
def create_azure_storage_queue_clients(
    ...
    for queue_info in monitoring_queues:
        ...
        client = QueueClient(
            queue_info.storage_account_url, queue_name=queue_info.queue_name, credential=default_credential
        )
...
def create_metric_gauge(monitoring_queues: List[MonitorQueueInformation]| None, meter: Meter) -> dict[str, Gauge]:
    ...
    for queue_info in monitoring_queues:
        ...
        gauge = meter.create_gauge(name=gauge_name, description=gauge_description, unit="messages")
